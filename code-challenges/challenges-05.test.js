...
const templateWithJQuery = () => {
  starWarsPeople.forEach(person => {
    ...
    let $clone = $(`<section>${html}</section>`).clone();
...
You are making a grocery list for ingredients needed in the gruffalo crumble recipe, below. Rather than taking the entire recipe, you only want a list of the word names.

Write a function named listFoods that takes in the recipe and returns a new array of the food words without any amount or units. Just the name. For example, '1 cup flour' will return 'flour'.
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((item) => {
    if (item.includes('medium')) {
      result.push(item.slice(15, item.length));
    } else if (item.includes('pounds')) {
      result.push(item.slice(9, item.length));
    } else if (item.includes('gallons')) {
      result.push(item.slice(10, item.length));
    } else if (item.includes('cups')) {
      result.push(item.slice(8, item.length));
